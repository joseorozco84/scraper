...
def get_links():
    ...
    for x in tqdm(range(last_page)): #00:02
        url_start = f'https://www.metacritic.com/browse/games/release-date/available/ps5/metascore?page={actual_page}'
        ...
        actual_page +=1
    print(f'{len(links_list)} juegos encontrados para scrapear')
    return links_list
...
def get_content():
    ...
    for x in tqdm(get_links()): # 02:31, 1.5it/s(requests) vs 00:51, 4.6it/s(session) lista de ps4 demora 49:47
        r = s.get(x, headers=header) #headers evita error 400
        home = r.content.decode('utf-8')
        parser = html.fromstring(home)
        
        if r.status_code > 200: #omite la url offline
